...
2. Transcoding can be an error-prone and somewhat unreliable process [fn:4] which is leading to many users seeing transcoding or conversion errors when attempting to play videos.
3. [ ] Transmitting transcoded media streams can actually /increase/ the network bandwith Husage of compared to sending the raw stream depending on the output stream attributes.
...
So how do we ensure /all/ users meet with this scenario /every/ video? We can convert all our media well in advance of when they're played to make sure each of the four requirements are satisfied. We can do this no matter what device or app they're playing their media on (so long as we know their Direct Play requirements). Here's a visualization of the conversion steps needed to accomplish this for /all/ Google Chromecast devices[fn:6]:

#+begin_center
#+attr_html: :width 1000px
[[file:container_subtitle_conversion_graph.png]]
#+end_center

#+begin_center
#+attr_html: :width 1000px
[[file:video_audio_conversion_graph.png]]
#+end_center

*** direct-play-nice

In order to make it easier to ahead-of-time convert all media to Direct Play Compatible video formats, I created a CLI utility written in Rust called [[https://github.com/ns-mkusper/direct-play-nice][direct-play-nice]] which uses [[https://www.ffmpeg.org/][FFmpeg]] to convert any video file to a set of video, audio and subtitle codecs which satisfy the Direct Play requirements of all streaming devices you specify (you can also specify 'all' for the --streaming-devices argument):
...
By converting our media files to Direct-Play-compatible versions we ensure that Direct Play scenario is encountered by every user each time they play any media on our streaming server.
...
Most of users /never/ want to think about the actual video files or streams they're watching. All they care about is hitting the play button and getting HQ video to play uninterrupted. Most would give up without even selecting a different version of the video, since that's a lot less intuitive than selecting the quality of the video.
